...
# 공고문의 "서류전형 평가기준"에 자기소개서로 무엇을 보는지 적어 두는 기관이 있다
# (근로복지공단: "조직이해/지원동기, 직무이해/자기개발, … (각 문항별 500자 이내)").
# 이것은 평가 영역이지 문항이 아니다. 실제 문항 문장은 입사지원 사이트에 있다.
...
def extract_areas(text: str) -> list[str]:
    """공고문에 적힌 자기소개서 평가 영역을 읽는다.

    문항이 아니다. "조직이해/지원동기, 직무이해/자기개발, …"은 자기소개서에서
    무엇을 보는지 적어 둔 것이고, 물어보는 문장은 입사지원 사이트에 있다. 한때
    이것을 문항 칸에 넣었는데, 값을 못 뽑았으면 비워 두어야 할 자리에 옆에 있던
    다른 정보를 채워 넣은 것이라 되돌렸다. 참고용으로 전형 메모에만 적는다.
    """
    flat = normalize_cell(text)
